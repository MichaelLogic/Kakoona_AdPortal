...
class AvatarGrffk < ActiveRecord::Base
  ...
  def self.finalize_and_cleanup(id)
    avatar_grffk = AvatarGrffk.find(id)
    direct_upload_url_data = DIRECT_UPLOAD_URL_FORMAT.match(avatar_grffk.direct_upload_url)
    s3 = AWS::S3.new
    
    if avatar_grffk.post_process_required?
      avatar_grffk.upload = URI.parse(URI.escape(avatar_grffk.direct_upload_url))
    else
      paperclip_file_path = "avatar_grffks/uploads/#{id}/original/#{direct_upload_url_data[:filename]}"
      s3.buckets[BUCKET_NAME].objects[paperclip_file_path].copy_from(direct_upload_url_data[:path])
    end

    avatar_grffk.processed = true
    avatar_grffk.save
